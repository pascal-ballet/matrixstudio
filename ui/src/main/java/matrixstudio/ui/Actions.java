...
public class Actions {
	...
	private void createMenuBarActions() {
		...
		addAction("MenuBar", new Action.Container("MenuBar", getAction("File"), getAction("Edit"), getAction("Matrix"), getAction("Task")));
	...
	private void createFileAction() {
		...
		if (Basics.isMac()) {
			addAction("File", new Action.Container("File", getAction("NewSimulation"), separator, getAction("OpenSimulation"),
					getAction("SafeOpenSimulation"), separator, getAction("SaveSimulation"), getAction("SaveSimulationAs")));
		}
		else {
			addAction("File", new Action.Container("File", getAction("NewSimulation"), separator, getAction("OpenSimulation"),
					getAction("SafeOpenSimulation"), separator, getAction("SaveSimulation"), getAction("SaveSimulationAs"), separator,
					getAction("About"), separator, getAction("Quit")));
	...
	private void createMatrixAction() {
					...
					public void notified(Notification notification) {
						if (matrixField.getChecked().size() > 0) {
							...
							if (xField.getValue() == null)
								xField.setValue(matrix.getSizeX());
							if (yField.getValue() == null)
								yField.setValue(matrix.getSizeY());
							if (zField.getValue() == null)
								zField.setValue(matrix.getSizeZ());
						}
					}
				});
		...
		addAction("Present", new Action.Container("Present") {
			@Override
			public List<Action> getActions() {
				List<Action> actions = new ArrayList<>();
				for (Matrix matrix : ui.getModel().getMatrixList()) {
					actions.add(new Action.Stub(matrix.getName(), Action.STYLE_DEFAULT | Action.STYLE_BOOLEAN_STATE) {

						@Override
						public boolean getState() {
							return ui.isShellOpenForMatrix(matrix);
						}

						@Override
						public int run(ActionMonitor monitor) {
							if (ui.isShellOpenForMatrix(matrix)) {
								ui.closeShellForMatrix(matrix);
							} else {
								ui.openShellForMatrix(matrix);
							}
							return STATUS_OK;
						}
					});
		...
		addAction("Matrix", new Action.Container("Matrix", getAction("Resize"), getAction("Present")));
	...
	private void createTaskAction() {
				...
				taskField.addListener(new NotificationListener() {
					...
					public void notified(Notification notification) {
						if (taskField.getChecked().size() > 0) {
							...
							if (xField.getValue() == null)
								xField.setValue(task.getGlobalWorkSizeX());
							if (yField.getValue() == null)
								yField.setValue(task.getGlobalWorkSizeY());
							if (zField.getValue() == null)
								zField.setValue(task.getGlobalWorkSizeZ());
						}
					}
				});
		...
		addAction("Task", new Action.Container("Task", getAction("Resize")));
