...
public class MatrixTabController extends Controller<Model>{
	...
	private Runnable matrixesRefresh = new Runnable() {
		public void run() {
			if ( simulator.isStarted() == true ) {
				...
				matrixStudio.getDisplay().timerExec(60, this);
			}
		}
	};
	...
	private ActionMonitor simulationMonitor = new ActionMonitor() {
		...
		public void done() {
			Display display = matrixStudio.getDisplay();
			if ( !display.isDisposed() ) {
				display.asyncExec(matrixesFinalRefresh);
			}
		}
	...
	public MatrixTabController(MatrixStudio matrixStudio) {
		this.matrixStudio = matrixStudio;
		this.simulator = matrixStudio.getSimulator();
	...
	public CompositeField createFields() {
		...
		matricesListField.addAction(new Action.Stub(null, resources.getImage("paint_stylo.png"), Action.STYLE_BUTTON){			
			...
			public int run(ActionMonitor monitor) {
				// File chooser
				FileDialog fd = new FileDialog(matrixStudio.getShell(), SWT.OPEN);
		        fd.setText("Open");
		        fd.setFilterPath("C:/");
		...
		matricesListField.addListener(new NotificationListener() {
			public void notified(Notification notification) {
				if ( BasicsUI.NOTIFICATION_SELECTION.equals(notification.name) ) {
					...
					matrixStudio.setSelection(matricesListField.getSingleSelection());
				}
			}
		});
		...
		matricesComposite.addAction(new Action.Stub(null, resources.getImage("run_tool.gif"), Action.STYLE_BUTTON){
			...
			@Override
			public int run(ActionMonitor monitor) {
				if(matrixStudio.isCompiled()==false) {
					if ( matrixStudio.saveModel(false) ) { 
						final boolean succeeded = simulator.compileKernelCode();
						matrixStudio.setCompiled(succeeded);
					} else {
				...
				if(matrixStudio.isCompiled() == true) {
					simulator.start(simulationMonitor);
					matrixStudio.getDisplay().timerExec(60, matrixesRefresh);
				}
				return Action.STATUS_OK;
			}
		});
		...
		matricesComposite.addAction(new Action.Stub(null, resources.getImage("bouton_step.png"), Action.STYLE_BUTTON){
			...
			public int run(ActionMonitor monitor) {
				if(matrixStudio.isCompiled()==false) {
					matrixStudio.setCompiled(simulator.compileKernelCode());
				}
		...
		matricesComposite.addAction(new Action.Stub(null, resources.getImage("stop.gif"), Action.STYLE_BUTTON){
			...
			public int run(ActionMonitor monitor) {
				simulator.stop();
				matrixStudio.setCompiled(false);
				return Action.STATUS_OK;
			}
		});
		...
		matricesComposite.addAction(new Action.Stub("+"){
			...
			public int run(ActionMonitor monitor) {
				...
				matrixStudio.log("Refresh step is " + simulator.getRefreshStep());
				return Action.STATUS_OK;
			}
		});
		...
		matricesComposite.addAction(new Action.Stub("Video"){
			...
			public int run(ActionMonitor monitor) {
				if(recordingMPEG == false) {
					FileDialog dialog = new FileDialog(matrixStudio.getShell(), SWT.SAVE);
					...
					try {
						...
						matrixStudio.log("Animation will be saved to file '"+resultFilename+"'.");
					} catch (Exception e) {
						matrixStudio.log("Can't save animation to file '"+resultFilename+"'.\n"+" Exception:"+e.getMessage());
						e.printStackTrace();
					}
					return Action.STATUS_OK;					
		...
		matricesComposite.addAction(new Action.Stub("Img", Action.STYLE_BUTTON | Action.STYLE_BOOLEAN_STATE){
			...
			public int run(ActionMonitor monitor) {
				if(recordingPNG == false) {
					...
					try {
						startRecordPNG(file);
						matrixStudio.log("Images will be saved to file '"+resultFilename+"_matrixName_timeStep.png'.");
					} catch (Exception e) {
						matrixStudio.log("Can't save animation to file '"+resultFilename+"'.\n"+" Exception:"+e.getMessage());
						e.printStackTrace();
					}
