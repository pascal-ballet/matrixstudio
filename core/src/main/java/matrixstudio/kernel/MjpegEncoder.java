...
public class MjpegEncoder
{
   ...
   private long AVIStreamHeaderOffset = -1;
   public MjpegEncoder(File aviFile, int width, int height, double framerate, int numFrames) throws Exception {
       ...
       AVIStreamHeaderOffset = aviChannel.position();
   ...
   public void addImage(Image image) throws Exception { // Must be changed to take SWT Image instead of AWT Image
   ...
   public void finishAVI() throws Exception {
       ...
       // Adapt the AviStreamHeader
       raf.seek(AVIStreamHeaderOffset);
   ...
   {
       byte[] fcc = new byte[]{'i','d','x','1'};
       int cb = 0;
       ArrayList<AVIIndex> ind = new ArrayList<AVIIndex>();

       void addAVIIndex(int dwOffset, int dwSize) {
